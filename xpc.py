...
############################################################################################################
    ...
    def getPOSI(self, ac = 0):
        ...
        result = struct.unpack(b"<4sxBffffffffff", resultBuf)
...
############################################################################################################
    def getDREFs(self, drefs):
        ...
        for dref in drefs:
            fmt = bytes("<B{0:d}s".format(len(dref)),'utf-8')
    ...
    def sendDREFs(self, drefs, values):
        ...
        for i in range(len(drefs)):
            ...
            if hasattr(value, "__len__"):
                ...
                fmt = bytes("<B{0:d}sB{1:d}f".format(len(dref), len(value)),'utf-8')
                buffer += struct.pack(fmt, len(dref), dref, len(value), value)
            else:
                fmt = bytes("<B{0:d}sBf".format(len(dref)), 'utf-8')
...
############################################################################################################
    ...
    def sendTEXT(self, msg, x = -1, y = -1):
        ...
        msgLen = len(msg)
        bmsgLen = bytes(str(msgLen),'utf-8')
        buffer = struct.pack(b"<4sxiiB" + bmsgLen + b"s", b"TEXT", x, y, msgLen, msg)
        self.sendUDP(buffer)
############################################################################################################
